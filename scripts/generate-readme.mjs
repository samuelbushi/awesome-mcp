...
const catalogEntriesDirectory = path.join(catalogRootDirectory, 'servers');
...
function listEntryFiles(directoryPath) {
  ...
  for (const directoryEntry of directoryEntries) {
    ...
    } else if (directoryEntry.isFile() && directoryEntry.name === 'server.json') {
...
const readmeLines = [];
readmeLines.push('# Awesome MCP');
readmeLines.push('');
readmeLines.push('A curated, PR-friendly directory maintained by WunderCorp. Entries live as JSON files under `servers/<category>/<slug>/server.json`. The README is generated from those files.');
readmeLines.push('');
...
if (orderedCategoryNames.length === 0) {
  readmeLines.push('No MCP servers have been submitted yet.');
  readmeLines.push('');
}
